...
from .devices import Device, device_by_mac, devices_by_owner
...
def run(
    ...
    *,
    ...
    try:
        devices = list(devices_by_owner(owner))
        if not devices:
...
def _format_client_record(client_info: dict[str, object]) -> dict[str, object]:
    ...
    registered = bool(device_by_mac(mac if isinstance(mac, str) else None))
    mac_value = mac if isinstance(mac, str) else None
    matched_device = device_by_mac(mac_value)
    ...
    record: dict[str, object] = {
        ...
        "last_seen": _format_timestamp(client_info.get("last_seen")),
...
def list_non_registered_active_devices(*, print_fn=print) -> None:
    ...
    try:
        clients = service.list_active_clients()
        unknown_clients = [ci for ci in clients if not device_by_mac(ci.get("mac"))]
