...
from ubiquiti import run_cli
from ubiquiti.cli import (
    list_active_devices,
    list_devices,
    list_non_registered_active_devices,
)
from ubiquiti.cli import main as cli_main
from ubiquiti.devices import Device
...
def test_cli_locks_devices(monkeypatch, capsys):
    ...
    monkeypatch.setattr("ubiquiti.cli.devices_by_owner", lambda owner: devices)
    monkeypatch.setattr("ubiquiti.cli.UniFiClient", DummyClient)
    monkeypatch.setattr("ubiquiti.cli.FirewallManager", DummyFirewallManager)
...
def test_cli_requires_owner(monkeypatch):
    monkeypatch.setattr("ubiquiti.cli.UniFiClient", DummyClient)
    monkeypatch.setattr("ubiquiti.cli.FirewallManager", DummyFirewallManager)
    monkeypatch.setattr("ubiquiti.cli.devices_by_owner", lambda owner: [])
...
def test_cli_list_devices(monkeypatch, capsys):
    ...
    monkeypatch.setattr("ubiquiti.cli.NetworkDeviceService", lambda client: service)
...
def test_cli_list_active_devices(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "ubiquiti.cli.lookup_mac_vendor",
...
def test_cli_main_list_active(monkeypatch, capsys):
    monkeypatch.setattr(
        "ubiquiti.cli.list_active_devices",
...
def test_cli_main_list_non_registered(monkeypatch, capsys):
    monkeypatch.setattr(
        "ubiquiti.cli.list_non_registered_active_devices",
        lambda print_fn=print: print_fn(json.dumps({"clients": []})),
    )
